...
/**
 * Instances of ConfigurationParameter are metadata about parameters. Parameters are described as having a certain type
 * (which can be hierarchically described), as being required or not, and with a default value (for most types). <br>
 * Parameters to a program are typically described in an XML file, and there is a method
 * <code>readParameterMetadata</code> to read in such a file and return a collection of ConfigurationParameters.
 * Parameters to a module are typically constructed statically in the class of the module itself.
 * <p/>
