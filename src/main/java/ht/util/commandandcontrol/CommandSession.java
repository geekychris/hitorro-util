...
import ht.util.io.TeeOutputStream;
...
import static ht.util.core.Env.getBin;
import static ht.util.core.Env.getHome;
...
public class CommandSession implements Runnable {
    ...
    public CommandSession() {
        ...
        try {
            ...
            Log.commands.error("Unable to open cli buffer for user system %s %e", e, e);
    ...
    public static boolean executeToLog(final String action, final String params) throws JobExecutionException {
        ...
        try {
            JVS jvs = CommandArgs.getParameters(params, true, true);
            InputStream is = null;
            ...
            cs.m_os = FileUtil.getBufferedFileOutputStream(FileUtil.getUniqueFileName(Env.getLogDir(), "startupscriptlog"));
    ...
    private static void copyCommandToFile(String command,
        ...
        if (session.m_scriptOs != null) {
            try {
                ...
                session.m_scriptOs.write(Constants.NewLineChar);
            } catch (IOException e) {
    ...
    private static JVS getArgs(String rest) {
        ...
        try {
            CommandArgs.parseArgs(rest, true, true, map);
        } catch (ParseException e) {
            Log.commands.error("Parsing error %s %e", e, e);
    ...
    public static List<GenericKeyValue> historyList(@DebugArgAno(keyName = "session",
            description = "session",
            defaultValue = "",
    ...
    public List<GenericKeyValue> getVars() {
    ...
    public void addExecutedCommand(String command, JVS args, String rawArgs) {
        ...
        if (userHistoryBuffer != null) {
            try {
                ...
                Log.commands.error("Unable to write to cli history %s %e", e, e);
            }
        }
        if (addMe) {
    ...
    public List<GenericKeyValue> getCommandsAsKeyValue() {
        List<GenericKeyValue> list = new ArrayList<GenericKeyValue>();
        int i = 0;
        for (CommandMap map : m_commands) {
            list.add(new GenericKeyValue(Integer.toString(i++), StringUtil.strcat(map.getCommand(), " ", map.getRawArgs())));
        }
        return list;
    }
    ...
    public void run() {
        ...
        try {
            ...
            Log.commands.error("The following exception occured %s %e", e, e);
        } catch (PropaccessError e) {
            Log.commands.error("The following exception occured %s %e", e, e);
        } finally {
            if (this.m_closeConnectionOnExit) {
                Log.commands.debug("About to flush and close connection");
                try {
                    m_os.flush();
                } catch (IOException e) {
                    Log.util.error("Exception %s %e", e, e);
    ...
    public void processTerminal(StringBuilder builder,
        ...
        Log.commands.debug("Processing standard terminal response.");
        ...
        Log.commands.debug("About to exit session");
    }
    ...
    public void executeStartupScript(File file) throws PropaccessError {
        if (!file.exists()) {
            Log.commands.info("Startup Script does not exist");
        }
    ...
    public void executeScript(CommandSession session, File file) throws PropaccessError {
        ...
        try {
            if (!file.exists()) {
                Log.commands.info("Script does not exist %s", file.getAbsolutePath());
            }
            in = FileUtil.getBufferedFileInputStream(file);
        } catch (FileNotFoundException e) {
            Log.commands.error("Unable to execute script %s %e", e, e);
        }
        if (in != null) {
            StringBuilder builder = new StringBuilder();
